...
class TDthree:
    ...
    def take_action(self, state):
        ...
        action = (action + self.sigma * np.random.normal(0, self.action_bound, size = self.action_dim)).clip(-self.action_bound, self.action_bound)
        return action
    ...
    def update(self, transition_dict):
        ...
        with torch.no_grad():
            ...
            q_targets = rewards + self.gamma * torch.min(next_q_values[0],next_q_values[1]) * (1-dones)
        ...
        if self.eps % self.policy_freq == 0:
            actor_loss = -torch.mean(self.critic(states, self.actor(states))[0])
...
agent = TDthree(state_dim, hidden_dim, action_dim, action_bound, sigma, actor_lr, critic_lr, tau, gamma, device)
